...
public class ProdutoForm extends JFrame implements ActionListener {
	...
	private String imgIco = "C:\\Users\\DESENVOLVIMENTO\\Desktop\\logic\\FPOO\\wk\\Projeto\\assets\\cafe.jpg";
	...
	ProdutoForm() {
		...
		rotulos = new JLabel("Cód | Nome | Estoque | Fornecedor | Vencimento | Preço | Status:");
	...
	private void cadastrar() {
		if (tfcodigoProduto.getText().length() != 0 && tfestoque.getText().length() != 0
			...
			double pU;
			try {
				pU = Double.parseDouble(df.parse(tfprecoUnitario.getText()).toString());
