...
public class OrcamentoForm extends JDialog implements ActionListener {
	...
	private JLabel Lid, Lfornecedor, Lproduto, Lpreco, LmaisBarato;
	...
	private void preencherAreaDeTexto() {
		...
		for (Orcamento m : OrcamentoProcess.orcamentos) {
			tableModel.addRow(new String[] {m.getId("s"), m.getFornecedor(), m.getProduto(), m.getPreco("s"), m.getMaisBarato("s")});
		}
	}
	...
	public void busca() {
		...
		if (isNumeric) {
			...
			if (OrcamentoProcess.orcamentos.contains(orca)) {
				...
				maisBarato.setText(OrcamentoProcess.orcamentos.get(indice).getMaisBarato("s"));
